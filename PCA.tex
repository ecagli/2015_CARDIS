...
\lambda_i =& \hat{\mathrm{var}}(\sum_{t_1=1}^D \XXX[t_1]\AAlpha_i[t_1]) = \sum_{t_1=1}^D\sum_{t_2=1}^D \hat{\mathrm{cov}}(\XXX[t_1]\AAlpha_i[t_1], \XXX[t_2]\AAlpha_i[t_2])=\\
...
\end{align}
where $\covmat_{t_1}$ indicates the $t_1$-th row of $\covmat$ and the last line is justified by the fact that $\AAlpha_i$ is an eigenvector of $\covmat$, with $\lambda_i$ its corresponding eigenvalue. The result of this computation is quite obvious, since $\parallel \AAlpha_i\parallel=1$, but it evidences the contribution of each time sample in the information held by the PC. This makes us introduce the following definition:
\begin{definition}
...
\end{equation}
\end{definition}
It may be observed that the sum over all the trace samples ELVs of a PC equals the EGV of the considered PC.\\
Actually, the ELV selection method, in analogy with the EGV one, consists in fixing a a threshold value $\beta$ for the wished {\em Cumulative Explained Local Variance} and in constructing the set 
\begin{equation}
S = \{(i_1, t_1), \dots , (i_{\newTraceLength},t_{\newTraceLength})\} 
\end{equation}
