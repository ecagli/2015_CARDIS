...
\end{definition}
\begin{figure}
\includegraphics[width=0.31\textwidth]{figures/firstPC.pdf} 
\includegraphics[width=0.31\textwidth]{figures/secondPC.pdf} 
\includegraphics[width=0.31\textwidth]{figures/thirdPC.pdf} \\
\includegraphics[width=0.31\textwidth]{figures/fourthPC.pdf} 
\includegraphics[width=0.31\textwidth]{figures/fifthPC.pdf} 
\includegraphics[width=0.31\textwidth]{figures/sixthPC.pdf} 
\caption{The first six PCs.}\label{fig:6components}
\end{figure}
It may be observed that the sum over all the trace samples ELVs of a PC equals the EGV of the considered PC. If we operate such a sum for each PC in a cumulative way and in decreasing order over the coefficients starting from the one that holds the maximal ELV, we obtain a complete vision of the trend that each component follows to achieve its EGV. As we can see in Fig.~(\ref{fig:ELVcumulative}), where such cumulative ELVs are represented, the first 3 components are much slower in achieving their final EGV, while the $4^\text{th}$, the $5^\text{th}$ and the $6^\text{th}$ achieve a large part of their final EGVs in a few points. This makes them be more expressive then the three leading PCs, if restricted to a small number of coefficient. To interpret this result and validate the meaning of the ELV, it suffices to look at the form the such components (Fig.~(\ref{fig:6components})): the leading ones are very influenced by the clock, while the latests are well localised over the leaking points.
