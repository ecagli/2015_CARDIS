...
=& \sum_{t_1=1}^D \AAlpha_i[t_1]\cdot \lambda_i\AAlpha_i[t_1]= \sum_{t_1=1}^D  \lambda_i \AAlpha_i[t_1]^2 \label{eq:toJustify}
\end{align}
where $\covmat_{t_1}$ denotes the $t_1$-th row of $\covmat$ and \eqref{eq:toJustify} is justified by the fact that $\AAlpha_i$ is an eigenvector of $\covmat$, with $\lambda_i$ its corresponding eigenvalue. The result of this computation is quite obvious, since $\parallel \AAlpha_i\parallel=1$, but it evidences the contribution of each time sample in the information held by the PC. This makes us introduce the following definition:
\begin{definition}
